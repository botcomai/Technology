...
function highlightActiveLink() {
  ...
  const financePages = ["wallet.html", "transactions.html", "orders.html", "developer.html"];
  ...
  const othersPages = ["schedule.html", "afa.html", "ecards.html"];
...
// Safety Hide (Max 3 seconds)
setTimeout(() => { if (window.hideLoader) window.hideLoader(); }, 3000);
...
document.addEventListener("click", (e) => {
    ...
    if (link && link.href && !link.href.includes("#") && !link.target && !link.href.startsWith("javascript:") && !link.onclick) {
        // Prevent showing loader for same-page fragments or special actions
...
document.addEventListener("DOMContentLoaded", async () => {
  // 1. Initial UI setup
  injectLoader();
  window.showLoader(); // Show by default until init is done
  ...
  // 3. Cleanup - Hide as soon as possible
  window.hideLoader();
});
