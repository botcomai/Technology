...
async function loadRecentOrders(page = 1) {
  ...
  orders.forEach(order => {
    ...
    tbody.innerHTML += `
      <tr>
        <td style="text-align:center; white-space:nowrap;"><input type="checkbox" class="row-checkbox" value="${order.id}" onclick="handleRowCheckboxChange(this)"></td>
        <td style="font-weight:800; color:#2563eb; white-space:nowrap;">${(order.network || 'ORD').split('-')[0].split(' ')[0].toUpperCase()}-${String(order.order_number || 0).padStart(2, '0')}</td>
        <td style="white-space:nowrap;">
            ...
            <div style="font-size:11px; color:#64748b; margin-top:2px;">Recip: ${order.phone}</div>
        </td>
        <td style="white-space:nowrap;">
        ...
        <td style="color:var(--text-muted); font-size:12px; font-family:monospace; white-space:nowrap;">
    ...
    data.forEach(tx => {
        ...
        tbody.innerHTML += `
            <tr>
                ...
                <td><strong style="font-size:14px;">₵${Number(tx.amount).toFixed(2)}</strong></td>
...
async function loadDataPlans() {
    ...
    const baseProductKeys = ['data_per_gb', 'afa_premium', 'afa_normal'];
    const basePlans = plans.filter(p => baseProductKeys.includes(p.product));
    const bundlePlans = plans.filter(p => !baseProductKeys.includes(p.product));
    ...
    sortedGroups.forEach(g => {
        ...
        tbody.innerHTML += `
            <tr style="${g.is_in_stock ? '' : 'opacity:0.6;'}">
                <td style="font-weight:600; white-space:nowrap; color:white;">${netName}</td>
                <td style="white-space:nowrap; color:var(--text-muted);">${g.plan_name} ${stockBadge}</td>
                <td style="white-space:nowrap;"><span style="color:var(--blue); font-weight:700;">${g.gb_size > 0 ? g.gb_size : ''} GB</span></td>
                <td style="white-space:nowrap; color:var(--text-muted);">${g.validity}</td>
                <td style="white-space:nowrap;">
    ...
    pageTxs.forEach(tx => {
        ...
        tbody.innerHTML += `
            <tr>
                ...
                <td style="white-space:nowrap; font-family:monospace; color:#3b82f6; font-weight:700; font-size:13px; letter-spacing:0.5px;">${ref}</td>
                <td style="white-space:nowrap;"><strong style="font-size:15px;">₵${Number(tx.amount).toFixed(2)}</strong></td>
                <td style="white-space:nowrap;">${statusBadge}</td>
                <td style="white-space:nowrap; text-align:right;">
                    ${isPending ? `
    ...
    tickets.forEach(t => {
        ...
        tbody.innerHTML += `
            <tr>
                ...
                <td><div style="font-size:13px; max-width:250px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;" title="${(t.issue || '').replace(/"/g, '&quot;')}">${shortIssue}</div></td>
                <td>${imgBadge}</td>
                <td><span style="background:${statusBg}; color:${statusColor}; font-weight:600; font-size:11px; padding:4px 8px; border-radius:4px; text-transform:uppercase; letter-spacing:0.5px; border:1px solid ${statusBg}; white-space:nowrap;">${statusLabel}</span></td>
...
async function loadUsers() {
    ...
    try {
        const { data: usersData, error } = await supabase
            .from('users')
            .select('id, email, phone, first_name, last_name, role, wallet_balance, created_at, is_free_mode, balance_owed, merchant_id')
            .order('created_at', { ascending: false });
...
function renderUsersTable() {
    ...
    const data = window.allUsersData || [];
    if (data.length === 0) {
    ...
    tbody.innerHTML = data.map(u => {
        ...
        const fullName = `${fn} ${ln}`.trim() || 'Unknown';
        const role = u.role || 'client';
        const bal = Number(u.wallet_balance || 0).toFixed(2);
        const code = u.merchant_id || '—';
        
        let roleBadge = `<span style="background:rgba(255,255,255,0.05); color:var(--text-muted); padding:4px 8px; border-radius:4px; font-size:11px; text-transform:uppercase;">${role}</span>`;
        if (role === 'admin') roleBadge = `<span style="background:rgba(239,68,68,0.1); color:#ef4444; padding:4px 8px; border-radius:4px; font-size:11px; font-weight:700; text-transform:uppercase;">ADMIN</span>`;
        else if (role === 'vip_customer') roleBadge = `<span style="background:rgba(59,130,246,0.1); color:#3b82f6; padding:4px 8px; border-radius:4px; font-size:11px; font-weight:600; text-transform:uppercase;">VIP</span>`;
        else if (role === 'elite_agent') roleBadge = `<span style="background:rgba(16,185,129,0.1); color:#10b981; padding:4px 8px; border-radius:4px; font-size:11px; font-weight:600; text-transform:uppercase;">ELITE</span>`;
        else if (role === 'super_agent') roleBadge = `<span style="background:rgba(245,158,11,0.1); color:#f59e0b; padding:4px 8px; border-radius:4px; font-size:11px; font-weight:600; text-transform:uppercase;">SUPER</span>`;
        ...
        const fmBadge = u.is_free_mode 
          ...
          : '';
        const owed = (u.balance_owed && u.balance_owed > 0) ? `<div style="font-size:11px; color:#f59e0b; font-weight:bold; margin-top:2px;">Owes: ₵${Number(u.balance_owed).toFixed(2)}</div>` : '';

        return `
            <tr>
                <td style="white-space:nowrap;">
                    <div style="font-family:monospace; font-size:11px; color:var(--blue); font-weight:700; margin-bottom:4px;">${code}</div>
                    <div style="font-weight:600; color:white; display:flex; align-items:center;">${fullName} ${fmBadge}</div>
                    <div style="font-size:12px; color:var(--text-muted); margin-top:2px;">${u.email}</div>
                    <div style="font-size:11px; color:#64748b; margin-top:2px;">📞 ${u.phone || 'N/A'}</div>
                </td>
                <td style="white-space:nowrap;">${roleBadge}</td>
                <td style="white-space:nowrap;">
                  <strong style="font-size:15px; color:${bal > 0 ? '#10b981' : 'white'};">₵${bal}</strong>
                  ${owed}
                </td>
                <td style="white-space:nowrap; display:flex; gap:8px; align-items:center;">
                    <select onchange="updateUserRole('${u.id}', this.value)" style="background:black; border:1px solid var(--glass-border); color:white; padding:6px; border-radius:4px; outline:none; font-size:12px;">
                        <option value="client" ${role === 'client' ? 'selected' : ''}>Client</option>
                        <option value="vip_customer" ${role === 'vip_customer' ? 'selected' : ''}>VIP</option>
                        <option value="elite_agent" ${role === 'elite_agent' ? 'selected' : ''}>Elite</option>
                        <option value="super_agent" ${role === 'super_agent' ? 'selected' : ''}>Super Agent</option>
                        <option value="admin" ${role === 'admin' ? 'selected' : ''}>Admin</option>
                    </select>
                    <button onclick="openUserTransactionsModal('${u.id}', '${fullName.replace(/'/g, "\\'")}')" style="background:var(--blue); border:none; color:white; padding:6px 12px; border-radius:4px; font-size:12px; cursor:pointer;">Transactions</button>
                    <button class="btn-action" onclick="toggleFreeModeAdmin('${u.id}', ${u.is_free_mode}, '${u.email}')" style="background:rgba(255,255,255,0.05); border:1px solid var(--glass-border); color:white; padding:6px 10px; border-radius:4px; font-size:11px; cursor:pointer;">FM</button>
                    <button class="btn-action" onclick="openUserWalletModal('${u.id}', ${u.wallet_balance})" style="background:rgba(255,255,255,0.05); border:1px solid var(--glass-border); color:white; padding:6px 10px; border-radius:4px; font-size:11px; cursor:pointer;">±Bank</button>
                </td>
            </tr>
        `;
    }).join('');
            ...
            oBody.innerHTML = orders.map(o => {
                ...
                return `
                    <tr>
                        ...
                        <td style="font-family:monospace; color:#3b82f6; white-space:nowrap; font-weight:700;">${friendlyId}</td>
                        <td style="font-weight:600;">${o.data_plan || o.product || 'Bundle'} <span style="font-size:11px; color:var(--text-muted);">(${o.recipient_number || 'N/A'})</span></td>
                        <td><strong style="white-space:nowrap;">₵${Number(o.amount_charged || o.amount || 0).toFixed(2)}</strong></td>
                        <td style="white-space:nowrap;">${statusBadge}</td>
                    </tr>
                `;
            }).join('');
            ...
            wBody.innerHTML = txs.map(t => {
                ...
                return `
                    <tr>
                        ...
                        <td style="font-family:monospace; color:#3b82f6; white-space:nowrap;">${t.reference || '-'}</td>
                        <td style="white-space:nowrap;">${t.type || 'Deposit'}</td>
                        <td><strong style="color:${amtColor}; white-space:nowrap;">₵${Number(t.amount).toFixed(2)}</strong></td>
                        <td style="white-space:nowrap;">${t.balance_after ? `₵${Number(t.balance_after).toFixed(2)}` : '-'}</td>
                        <td style="white-space:nowrap;">${statusBadge}</td>
                    </tr>
                `;
            }).join('');
            ...
            setBody.innerHTML = settlements.map(s => {
                ...
                return `
                    <tr>
                        ...
                        <td><div style="font-weight:600; font-size:13px;">${name}</div></td>
                        <td><strong style="color:white;">₵${Number(s.amount_paid).toFixed(2)}</strong></td>
                        <td style="font-size:12px;">${(s.payment_method || '').toUpperCase()}<br><span style="color:var(--text-muted);">${s.reference || 'N/A'}</span></td>
                        <td>${badge}</td>
    ...
    try {
        const { error } = await supabase
            ...
            .update({ 
                ...
                // Automatically turn off free mode if balance is 0 and they were only in free mode due to debt
                is_free_mode: balanceNum > 0
            })
            .eq('id', userId);

        if (error) throw error;

        if (window.showSuccessPopup) window.showSuccessPopup("Updated", `Balance updated to ₵${balanceNum.toFixed(2)}`);
        else alert(`Balance updated to ₵${balanceNum.toFixed(2)}`);
        
        loadFreeModeData();
        loadUsers();
    } catch (err) {
        if (window.showErrorPopup) window.showErrorPopup("Failed", err.message);
        else alert("Failed to update balance: " + err.message);
    }
}
