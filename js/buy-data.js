...
async function makePayment() {
  ...
  for (const item of normalItems) {
    ...
    if (isFreeMode) {
      // Free Mode Logic (still in frontend for now, or could be moved)
      ...
      // API Logic (Secure Backend call)
      try {
        const response = await fetch(`${BACKEND_URL}/api/buy-data`, {
          method: 'POST',
        ...
        insertedOrderId = result.order_id;
        currentBalance  = result.new_balance;
      } catch (apiErr) {
        console.error('API Error:', apiErr.message);
        showCartToast(`Order failed: ${apiErr.message}`, 'error');
        continue; // Skip this order if it failed
      }
    }

    // Fulfillment is now handled internally by the Backend API for non-FreeMode orders.
    // If it was FreeMode, we might still want to trigger the old flow or update the backend.
    if (isFreeMode && insertedOrderId) {
      // (Optional) Trigger old Edge Function for Free Mode if needed
    }
...
if (selectedNetwork.startsWith('MTN') && insertedOrderId) {
  try {
    const edgeFnUrl = `${window.SUPABASE_URL}/functions/v1/fulfill-mtn-order`;
    const fulfillRes = await fetch(edgeFnUrl, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        phone: phone,
        data_size: `${bundle}GB`,  // bundle is already the numeric GB, e.g. '2' → '2GB'
        order_id: insertedOrderId
      })
    });
    const fulfillResult = await fulfillRes.json();
    console.log(`MTN Auto-fulfill result for order ${insertedOrderId}:`, fulfillResult);
  } catch (e) {
    console.warn('MTN auto-fulfillment failed (order will stay pending):', e.message);
  }
}
